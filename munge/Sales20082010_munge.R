...
summary(sales0810) #found that some observations have a 0 or negative sales value
t <- which(sales0810$SALE_VALUE < 1)
sales0810 = sales0810 [-t, ] #Excludes observations without sales value information

#Pulling the variables we WANT into a new, working dataframe.
sales0810$GARSQFT <- as.numeric(as.character(sales0810$GARAGESQFT)) #Converting factor to numeric
sales0810$SALE_SEASON <- factor(sales0810$SALE_QRT)
sales0810$SALE_MO <- factor(sales0810$SALE_MONTH)
dataNames <- names (sales0810)
VarsIWant <- which(dataNames %in% c("GARSQFT","SALE_SEASON", "SALE_MO","COUNTY_ID", "CITY", "ZIP", "ACRES_POLY", "HOMESTEAD", "TOTAL_TAX", "HOME_STYLE", "FIN_SQ_FT", "GARAGE", "YEAR_BUILT", "SALE_VALUE", "SALE_YR", "BEDS", "BATH", "MAX", "TRACTCE10", "BLDG_QUAL", "PARK_dist", "LAKE_dist", "SDNUM", "MCA3", "MCA5", "UNIQID", "Long_X", "Lat_Y", "SHOP_dist", "CBD_dist", "PIN"))
...
Sales20082010Clean = data.frame(workingdata)
write.dbf(Sales20082010Clean, "../Data/R2GIS/CleanData/Sales20082010.dbf" )