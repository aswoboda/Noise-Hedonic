...
#Diagnostics
plot(model.SaleValue1) #Normality issues once again #3156 large leverage
...
model.SaleValue2 <- lm (SALE_VALUE ~ COUNTY_ID + CITY + SALE_YR  + ACRES_POLY + HOMESTEAD + FIN_SQ_FT + YEAR_BUILT + MAX + PARK_dist + LAKE_dist + MCA3 + SHOP_dist + CBD_dist + SALE_MO +SALE_SEASO, data=workingdata)
...
model.logSaleValue1 <- lm (logSALE_VA ~ COUNTY_ID + CITY + SALE_YR  + ACRES_POLY + ACRES2 + HOMESTEAD + logFIN_SQ_ + YEAR_BUILT + logMAX + logPARK + logLAKE + MCA3 + logSHOP + logCBD + SALE_MO +SALE_SEASO, data=workingdata)
...
plot(model.logSaleValue1) #Normality improved

workingdata = workingdata[-c(3156), ] #Removed influential observations

###Dependent variable: Sales Value (log transformed) w/o MCA5, logLAKE, SDNUM
...
anova(model.logSaleValue1, model.logSaleValue2)
...
names(temp)
temp = temp[, c(2, 4)]
write.dbf(temp, "../Data/R2GIS/20052007logSales.dbf")
