...
model.logSaleValue1 <- lm (logSALE_VA ~ COUNTY_ID + CITY + factor(SALE_YR)  + ACRES_POLY+ HOMESTEAD + log(FIN_SQ_FT) + YEAR_BUILT + log(MAX) + PARK_dist + LAKE_dist + MCA3 + MED_INCOME+COLLEGE_di+ SHOP_dist + CBD_dist + SALE_MO, data=workingdata)
...
model.logSaleValue2 <- lm (logSALE_VA ~ COUNTY_ID + CITY + factor(SALE_YR)  + ACRES_POLY+ I(ACRES_POLY ^2) + HOMESTEAD + log(FIN_SQ_FT) + YEAR_BUILT + log(MAX) + PARK_dist + I(PARK_dist ^2) + LAKE_dist+ I(LAKE_dist ^2) + MCA3 + MED_INCOME + COLLEGE_di+ I(COLLEGE_di ^2) + SHOP_dist+ I(SHOP_dist^2) + CBD_dist + I(CBD_dist ^2)+ SALE_MO, data=workingdata)
...
model.logSaleValue3 <- lm (logSALE_VA ~ COUNTY_ID + CITY + factor(SALE_YR)  + (ACRES_POLY*CBD_dist) + I(ACRES_POLY ^2) + HOMESTEAD + log(FIN_SQ_FT) + YEAR_BUILT + (log(MAX)*CBD_dist) + PARK_dist + I(PARK_dist ^2) + LAKE_dist+ I(LAKE_dist ^2) + MCA3 + MED_INCOME + COLLEGE_di+ I(COLLEGE_di ^2) + SHOP_dist+ I(SHOP_dist^2) + I(CBD_dist ^2)+ SALE_MO, data=workingdata)
