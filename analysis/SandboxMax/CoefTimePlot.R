...
pdf("analysis/02LWRComparisons/LWRcoefPlotOverTime3_17b_Noise.pdf", height = 8, width = 12)
par(mfrow = c(2, 3))
for (i in bandwidths2plot) {
  myBandwidth = colnames(output[[myCoeff]])[i] # choose a bandwidth
  myX = DATAFRAME$TimePeriod[obs2run]
  myY = (output[[myCoeff]]*DATAFRAME$SALE_VALUE)[, myBandwidth]
  plot(myX, myY, cex = .2,
       xlab = "Time",
       ylab = "mfxNoise",
       main = myBandwidth)
...
###Test for Median band
names(output)
myCoeff = names(output)[2] # choose a coefficient to work with
colnames(output[[myCoeff]])
bandwidths2plot = c(2, 3, 4, 6, 8, 10)

pdf("analysis/02LWRComparisons/LWRcoefPlotOverTime3_17e_Median.pdf", height = 8, width = 12)
par(mfrow = c(2, 3))
for (i in bandwidths2plot) {
  myBandwidth = colnames(output[[myCoeff]])[i] # choose a bandwidth
  myX = DATAFRAME$TimePeriod[obs2run]
